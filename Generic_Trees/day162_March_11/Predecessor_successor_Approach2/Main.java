...
public class Main {
    ...
    public static void findPredecessorSuccessorOfNode(Node node, int tar) {
        if (node.data == tar) {
            predecessor = prev;
        }
        if (prev != null && prev.data == tar) {
            successor = node;
            return;
        }
        prev = node;
        for (Node child : node.children) {
            if (successor == null) {
                findPredecessorSuccessorOfNode(child, tar);
            }
        }
    }

    public static void findPredecessorSuccessorOfNode2(Node node, int tar) {
        if (state == 0) {
            if (node.data == tar) {
                state = 1;
            } else {
        ...
        for (Node child : node.children) {
            findPredecessorSuccessorOfNode2(child, tar);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        prev = null;
        state = 0;
        findPredecessorSuccessorOfNode2(root1, 20);
