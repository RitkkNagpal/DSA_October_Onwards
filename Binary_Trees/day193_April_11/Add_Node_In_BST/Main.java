...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int val = scn.nextInt();
        addNodeInBST(root, val);
        display(root);
        scn.close();
    }

    private static void addNodeInBST(Node node, int val) {
        Node nn = new Node(val, null, null);
        if (node.data > val) {
            if (node.left != null) {
                addNodeInBST(node.left, val);
            } else {
                node.left = nn;
            }
        } else {
            if (node.right != null) {
                addNodeInBST(node.right, val);
            } else {
                node.right = nn;
            }
        }
