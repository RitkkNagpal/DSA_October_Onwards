...
public class Main {
    ...
    private static int tileOfTree(Node node) {
        if (node == null) {
            return 0;
        }
        int leftSubtreeSum = tileOfTree(node.left);
        int rightSubtreeSum = tileOfTree(node.right);
        int currTilt = Math.abs(leftSubtreeSum - rightSubtreeSum);
        tilt += currTilt;
